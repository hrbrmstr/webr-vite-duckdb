...
export class OJSShorthandPlot extends LitElement {
	...
	render() {
		...
		const foreground = style.getPropertyValue('--foreground-color')
		const background = style.getPropertyValue('--background-color')
		const renderedPlot = this.chart && this.chart.plot({
			color: {
				legend: true
			},
			style: this.style
...
window.customElements.define('ojs-shorthand-plot', OJSShorthandPlot)
